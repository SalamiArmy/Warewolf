...
namespace Dev2.Studio.Core.Specs.IntellisenseSpecs
{
    ...
    public class IntellisenseProviderSteps
    {
        ...
        public IntellisenseProviderSteps(ScenarioContext scenarioContext) => this.scenarioContext = scenarioContext ?? throw new ArgumentNullException("scenarioContext");
        ...
        public void GivenIHaveTheFollowingVariableList(string variableList)
        {
            ...
            scenarioContext.Add("dataList", datalist);
        ...
        {
            scenarioContext["datalistOptions"] = p0.Split( new char[]{','});
        }
        ...
        public void GivenTheProviderUsedIs(string providerName)
        {
            ...
            scenarioContext.Add("IsInCalculate", providerName.Contains("Calc"));
            scenarioContext.Add("provider", provider);
        }
        ...
        public void GivenTheFilePathStructureIs(string pathStructure)
        {
            ...
            scenarioContext.Add("fileQueryHelper", fileQueryHelper);
        }
        ...
        IIntellisenseProvider GetProvider(string providerName)
        {
            ...
            switch (providerName.Trim())
            {
                case "Calculate":
                    ...
                    break;
                case "File":
                    var fileSystemIntellisenseProvider = new FileSystemIntellisenseProvider { FileSystemQuery = scenarioContext.Get<IFileSystemQuery>("fileQueryHelper") };
                    provider = fileSystemIntellisenseProvider;
                    scenarioContext.Add("isFileProvider", true);
                    break;
                case "DateTime":
        ...
        {
            var calc = scenarioContext.Get<bool>("IsInCalculate");

            var context = new IntellisenseProviderContext
            {
                CaretPosition = scenarioContext.Get<int>("cursorIndex"),
                CaretPositionOnPopup = scenarioContext.Get<int>("cursorIndex"),
                InputText = scenarioContext.Get<string>("inputText"),
                DesiredResultSet = calc ? IntellisenseDesiredResultSet.ClosestMatch : IntellisenseDesiredResultSet.Default,
                FilterType = scenarioContext.Get<enIntellisensePartType>("filterType"),
                IsInCalculateMode = calc
            };

            scenarioContext.Add("context", context);

            var provider = scenarioContext.Get<IIntellisenseProvider>("provider");
        ...
        {
            var inputText = scenarioContext.Get<string>("inputText");
        ...
        public void GivenTheSuggestionListAs(string p0)
        {
            ...
            provider.VariableList = new ObservableCollection<string>(scenarioContext["datalistOptions"] as IEnumerable<string>);
            var filterType = scenarioContext["filterType"] is enIntellisensePartType ? (enIntellisensePartType)scenarioContext["filterType"] : enIntellisensePartType.None;
            var caretpos = int.Parse(scenarioContext["cursorIndex"].ToString());
            var options = provider.GetSuggestions(scenarioContext["inputText"].ToString(), caretpos, true,filterType);
            ...
            Assert.IsTrue(all);
            scenarioContext["stringOptions"] = options;
        }
        ...
        {
            var context = scenarioContext.Get<IntellisenseProviderContext>("context");
            string result;

            if (scenarioContext.TryGetValue("isFileProvider", out bool isFileProvider))
            {
        ...
        { 
            var originalText =scenarioContext["inputText"].ToString();
            var caretpos = int.Parse(scenarioContext["cursorIndex"].ToString());
            if (option=="")
            {
                scenarioContext["stringResult"] = new IntellisenseStringResult(originalText,caretpos) ;
                return;
            }

            var options =scenarioContext["stringOptions"] as IEnumerable<string>;
        ...
        {
           var res =  scenarioContext["stringResult"] as IIntellisenseStringResult;
